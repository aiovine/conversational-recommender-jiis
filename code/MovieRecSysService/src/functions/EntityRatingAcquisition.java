...
public class EntityRatingAcquisition {
	...
	public void obtainEntityToRate() throws Exception {
		...
		String entityURI = service.getEntityToRatingByPopularEntities(Integer.parseInt(userID));
		
		boolean alreadyRated = false;
		ProfileService profileService = new ProfileService();
		JsonArray profile = profileService.getUserProfileWithSkips(Integer.parseInt(userID));
		...
		do {
			...
			for (Iterator<JsonElement> iterator = profile.iterator(); iterator.hasNext();) {
				...
				if (entityURI.equals(uri)) {
					...
					alreadyRated = true;
					entityURI = service.getEntityToRatingByPopularEntities(Integer.parseInt(userID));
					break;
				}
			}
	...
	public void putEntityToRate() throws Exception {
		...
		EntityService service = new EntityService();
		service.insertEntityToRate(Integer.parseInt(userID), entityURI, 0);
	}
...
System.out.println("properties: " + properties);
	...
	public void putEntityRating(int rating, String lastChange) throws Exception {
		...
		entityService.addEntityPreference(Integer.parseInt(userID), entityURI, rating, lastChange);
