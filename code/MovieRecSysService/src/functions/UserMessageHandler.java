...
public class UserMessageHandler {
	...
	public Reply handle() throws Exception {
		...
		DialogState state = dialogStateService.getDialogState(Integer.parseInt(userID));
		if (state == null) {
			state = new DialogState(Integer.parseInt(userID));
		}
		...
		if (!doQuestionnaire || state.getCurrentQuestionIndex() < 0) {
			if (INTERACTION_TYPE.equals("buttons")) {
				...
				try {
					dialogStateService.saveDialogState(Integer.parseInt(userID), state);
				} catch (Exception e) {
			...
			if (answerId != -1) {
				...
				questionnaireService.insertAnswer(Integer.parseInt(userID), questionId, answerId);
		...
		int newPRCycle = ps.getPagerankCycle(Integer.parseInt(userID));
		int newNumRecList = ps.getNumberRecommendationList(Integer.parseInt(userID));
	...
	private Reply handleButtonMessage(DialogState state, LogService logService) throws Exception {
		...
		// Details (Rate Entities)
			...
			boolean hasPositiveRating = (new ProfileService()).hasPositiveRating(Integer.parseInt(userID));
		...
		// Film raccomandato valutato positivamente
			...
			entityService.addRecommendedEntityPreference(Integer.parseInt(userID), entityService.getEntityURI(state.getEntityToRecommend()), rating, "user");
		...
		// I like, but...
			...
			entityService.setRefine(Integer.parseInt(userID), entityService.getEntityURI(entity));
		...
		// Details (Recommend Entities)
			...
			entityService.setDetails(Integer.parseInt(userID), entityService.getEntityURI(entity));
			System.out.println("*****entity: " + entity + "entityURI: " + entityService.getEntityURI(entity));
			logService.addEvent(EventType.RECOMMENDATION);
			logService.addEvent(EventType.QUESTION);
		...
		// Why?
			...
			entityService.setWhy(Integer.parseInt(userID), entityService.getEntityURI(entity));
			logService.addEvent(EventType.RECOMMENDATION);
			logService.addEvent(EventType.QUESTION);

		// Change - Refocus
		} else if (text.startsWith(emojis.get(EmojiCodes.ANGER))) {
			profileService.doRefocus(Integer.parseInt(userID));
			reply = new EntityRecommendationReply(entityRecommendation, 1, true, userID, messageID, state, logService, null, -1);
			logService.addEvent(EventType.QUESTION);
