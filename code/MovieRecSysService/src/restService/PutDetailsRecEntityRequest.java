...
public class PutDetailsRecEntityRequest {
								   	 ...
								   	 ) throws Exception 
	{
		...
		if (!entityURI.equalsIgnoreCase("null")) {
			asController.insertDetailsRecEntityRequestByUser(user_id, entityURI, number_recommendation_list, details);
			asController.putLastChange(user_id, "pagerank");
		}
		else {
			System.err.println("/putDetailsRecEntityRequest/Error - insertWhyRecEntityRequestByUser userID: " + user_id + " entityURI:" + entityURI);
		}		
		numberRatedEntities = asController.getNumberRatedEntities(user_id);		
		Gson gson = new Gson();
		String json = gson.toJson(numberRatedEntities);
			
		System.out.print("/putDetailsRecEntityRequest/userID: " + user_id + " entityURI:" + entityURI + " numberRecommendationList:" + numberRecommendationList + "/");
