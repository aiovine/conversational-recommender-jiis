...
public class PutDislikeRecEntityRating {
								   	 ...
								   	 ) throws Exception 
	{
		...
		if (!entityURI.equalsIgnoreCase("null")) {
			asController.insertDislikeRecEntityRatingByUser(user_id, entityURI, number_recommendation_list, r);
			asController.putLastChange(user_id, "pagerank");
		}
		else {
			System.err.println("/putDislikeRecEntityRating/Error - insertDislikeRecEntityRating userID: " + user_id + " entityURI:" + entityURI);
		}		
		numberRatedEntities = asController.getNumberRatedEntities(user_id);		
		Gson gson = new Gson();
		String json = gson.toJson(numberRatedEntities);
			
		System.out.print("/putDislikeRecEntityRating/userID: " + user_id + " entityURI:" + entityURI + " numberRecommendationList:" + numberRecommendationList + "/");
