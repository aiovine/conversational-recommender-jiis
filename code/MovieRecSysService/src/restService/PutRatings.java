...
public class PutRatings {
							   	 ...
							   	 @QueryParam("lastChange") String lastChange) throws Exception 
	{
		...
		if (!entityObjectUri.equalsIgnoreCase("null")) {
			asController.putEntityRatedByUser(user_id, entityObjectUri, r, lastChange);
			if (r.equals(1) || r.equals(2) ) {
				service.addEntityPreference(Integer.parseInt(userID), entityObjectUri, r, lastChange);
				
				asController.putLastChange(user_id, lastChange);
			}
		}
		else {
			System.err.println("Error - insertEntityRatedByUser userID: " + user_id + " entityURI:" + entityURI);
		}		
...
System.out.println("debug propertyType: " + propertyTypeURI);
            ...
            service.addPropertyPreference(Integer.parseInt(userID), property, propertyType, r, lastChange);
			asController.putPropertyRatedByUser(user_id, propertyType, property, r, lastChange);
