...
public class PutRecEntityRating {
								   	 ...
								   	 ) throws Exception 
	{
		...
		if (!entityURI.equalsIgnoreCase("null")) {
			asController.insertRecEntityRatedByUser(user_id, entityURI,number_recommendation_list, r, pos, pagerank_cicle, refine, refocus, botName, messageID, bot_timestamp, recommendatinsList, ratingsList);
			asController.putLastChange(user_id, "entity_rating");
		}
		else {
			System.err.println("/putRecEntityRating/Error - insertRecEntityRatingByUser userID: " + user_id + " entityURI:" + entityURI);
		}		
		numberRatedEntities = asController.getNumberRatedEntities(user_id);		
		Gson gson = new Gson();
		String json = gson.toJson(numberRatedEntities);
			
		System.out.print("/putRecEntityRating/userID: " + user_id + " entityURI:" + entityURI + " numberRecommendationList:" + numberRecommendationList + " refine:" + refine + " refocus:" + refocus + "/");
