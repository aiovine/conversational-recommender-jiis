...
public class PutRefineRecEntityRating {
								   	 ...
								   	 ) throws Exception 
	{
		...
		if (!entityURI.equalsIgnoreCase("null")) {
			asController.insertRefineRecEntityRatingByUser(user_id, entityObjectUri, number_recommendation_list, refine);
			//asController.putLastChange(user_id, "pagerank");
		}
		else {
			System.err.println("/putRefineRecEntityRating/Error - insertRefineRecEntityRating userID: " + user_id + " entityURI:" + entityURI);
		}		
		numberRatedEntities = asController.getNumberRatedEntities(user_id);		
		Gson gson = new Gson();
		String json = gson.toJson(numberRatedEntities);
			
		System.out.print("/putRefineRecEntityRating/userID: " + user_id + " entityURI:" + entityURI + " numberRecommendationList:" + numberRecommendationList + "/");
