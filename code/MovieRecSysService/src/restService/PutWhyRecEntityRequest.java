...
public class PutWhyRecEntityRequest {
								   	 ...
								   	 ) throws Exception 
	{
		...
		if (!entityURI.equalsIgnoreCase("null")) {
			asController.insertWhyRecEntityRequestByUser(user_id, entityURI, number_recommendation_list, why);
			//asController.putLastChange(user_id, "pagerank");
			asController.putLastChange(user_id, "entity_why");
		}
		else {
			System.err.println("/putWhyRecEntityRequest/Error - insertWhyRecEntityRequestByUser userID: " + user_id + " entityURI:" + entityURI);
		}		
		numberRatedEntities = asController.getNumberRatedEntities(user_id);		
		Gson gson = new Gson();
		String json = gson.toJson(numberRatedEntities);
			
		System.out.print("/putWhyRecEntityRequest/userID: " + user_id + " entityURI:" + entityURI + " numberRecommendationList:" + numberRecommendationList + "/");
