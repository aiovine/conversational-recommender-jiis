...
public class JaccardIndex {
	...
	public Map<String, Double> getJaccardIndex(int user_id) throws Exception{
        ...
        if (numberRatedItems == 0) {						//se non ci sono film valutati dall'utente
			String movieURIpopular = dbAccess.selectMovieToRatingByRandom(user_id);	//parti da un film random
	...
	public Map<String, Double> getJaccardDistance(int user_id) throws Exception{
        ...
        if(jaccardDistanceTopTwentyMap.size() < 1) {
        	int i = 0;
        	while(i<5) {
        	String movieURIpopula = dbAccess.selectMovieToRatingByRandom(user_id);	//parti da un film random
