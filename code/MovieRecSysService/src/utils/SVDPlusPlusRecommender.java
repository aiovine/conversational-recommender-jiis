...
public class SVDPlusPlusRecommender extends BiasedMFRecommender {
    ...
    protected void trainModel() throws LibrecException {
        for (int iterationStep = 1; iterationStep <= numIterations; iterationStep++) {
            ...
            for (int userIndex = 0; userIndex < numUsers; userIndex++) {
                ...
                factorVector.assign(new VectorAssigner() {
					@Override
					public double getValue(int index, double value) {
						return 0.0D;
					}
				});
                for (final Vector.VectorEntry vectorEntry : userVector) {
                    factorVector.assign(new VectorAssigner() {
						@Override
						public double getValue(int index, double value) {
							return impItemFactors.row(vectorEntry.index()).get(index) + value;
						}
					});
                }
                final double scale = Math.pow(userVector.getNumEntries(), -0.5);
                factorVector.assign(new VectorAssigner() {
					@Override
					public double getValue(int index, double value) {
						return value * scale;
    ...
    public double predict(int userIndex, int itemIndex) {
        ...
        final double scale = Math.sqrt(userVector.getNumEntries());
        if (scale > 0D) {
            factorVector.assign(new VectorAssigner() {
				@Override
				public double getValue(int index, double value) {
